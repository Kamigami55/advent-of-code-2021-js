...
function solve(draws, boards) {
  ...
  boards.forEach((board) => {
    let roundCanWinForBoard = Number.MAX_SAFE_INTEGER
    board.forEach((row) => {
      const roundCanWin = row.reduce(
        (max, val) => Math.max(max, hashTable.get(val) ?? Number.MAX_SAFE_INTEGER),
        0
      )
      roundCanWinForBoard = Math.min(roundCanWinForBoard, roundCanWin)
    })
    ...
    if (roundCanWinForBoard > winnerRound) {
      winnerBoard = board
      winnerRound = roundCanWinForBoard
    }
  })
